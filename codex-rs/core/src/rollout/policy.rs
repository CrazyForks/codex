...
fn should_persist_event_msg_compact(ev: &EventMsg) -> bool {
    matches!(
        ...
        Some(EventPersistenceMode::Compact)
    )
}
...
fn should_persist_event_msg_full_history(ev: &EventMsg) -> bool {
    matches!(
        event_msg_persistence_mode(ev),
        Some(EventPersistenceMode::Compact) | Some(EventPersistenceMode::FullHistory)
    )
}
...
fn event_msg_persistence_mode(ev: &EventMsg) -> Option<EventPersistenceMode> {
    match ev {
        ...
        | EventMsg::TurnAborted(_) => Some(EventPersistenceMode::Compact),
